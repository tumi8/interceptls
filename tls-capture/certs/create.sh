...
openssl req -new -config cert.conf -key server-key-rsa.pem -out server-cert-rsa.csr
...
openssl req -new -config cert.conf -key server-key-ec.pem -out server-cert-ec.csr
