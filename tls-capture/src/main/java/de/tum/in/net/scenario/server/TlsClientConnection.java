...
class TlsClientConnection implements Runnable {
    ...
    public TlsClientConnection(final Socket socket, final TlsServerFactory tlsServerFactory, final ResultListener publisher) {
        ...
        this.tlsServerFactory = Objects.requireNonNull(tlsServerFactory, "tlsServerFactory must not be null.");
    ...
    public void run() {
        ...
        try {
            ...
            if (Arrays.equals(answer, OK)) {
                ...
                publisher.publish(Severity.NOT_OK, new ScenarioResult(socket.getRemoteSocketAddress().toString(), received, sent));
            } else {
            ...
            publisher.publish(Severity.ERROR, new ScenarioResult(socket.getRemoteSocketAddress().toString(), "Error", e, tap));
        }
