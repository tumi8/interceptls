...
public class ResultUploaderTest {
  ...
  public void uploaderSavesResultsInCaseOfError() {
    ...
    ScenarioResult result =
        new ScenarioResultBuilder(Node.SERVER, "src", "dst").error(new Throwable());
    uploader.publish(id, result);

    ScenarioResult result2 =
        new ScenarioResultBuilder(Node.SERVER, "src2", "dst2").error(new Throwable());
    uploader.publish(id, result2);

    ScenarioResult result3 =
        new ScenarioResultBuilder(Node.SERVER, "src3", "dst3").error(new Throwable());
